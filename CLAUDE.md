...
- **Collection** — everything you own. Two views on it: All cards and Sets. In copy the word
  means only this set.
- **Folder** — a grouping you make yourself, by hand or by rule, and three that are always
  there: Favorites, Wishlist and the Pokédex. The `collections` table and the `/collections`
  routes are the folders you make; in copy they are all "Folders", never "collections".
