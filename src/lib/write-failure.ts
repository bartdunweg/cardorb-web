 ...
 *
 * A 400 keeps the API's own sentence: that one is about what was sent (a copy's finish, a CSV's
 * columns) and is the only refusal that can say something this app does not already know.
 */
const SAID: Record<number, string> = {
    ...
    409: "That changed somewhere else. Reload the page and try again.",
...
export function writeFailure(err: unknown): FailedWrite {
    ...
    if (err.status === 400 && err.message) return { ok: false, error: err.message };
