...
// Remembered a minute: the layout and a page both ask, on every screen.
export const getStats = () => remembered("stats", async (token) => (await api<{ stats: ApiStats }>("/stats", { token })).stats);
