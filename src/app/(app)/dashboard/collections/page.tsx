...
export default async function CollectionsPage() {
    const [{ collections, favoritesCount, wishlistCount }, facets] = await Promise.all([getMyCollections(), getFacets()]);

    return (
        <div className="flex flex-1 flex-col gap-6">
            <PageHeader title="Folders" subtitle="Group your cards the way you like." actions={<NewCollectionButton facets={facets} />} />
            <CollectionsGrid collections={collections} favoritesCount={favoritesCount} wishlistCount={wishlistCount} facets={facets} />
        </div>
    );
}
