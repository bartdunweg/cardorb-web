                ...
                {cards.map((card) => (
                    <li key={card.id} className="w-28 shrink-0 snap-start sm:w-32">
                        <CardTile
                            ...
                            // The tile's own gap and radius: a row this narrow sits tighter than the grid's.
                            className="gap-1.5 rounded-card"
                            picture={
                                <div className="relative aspect-card w-full overflow-hidden rounded-card bg-quaternary">
                                    {card.image_url ? (
                                        <CardImage src={card.image_high_url ?? card.image_url} alt="" width={160} className="object-cover" />
                                    ) : null}
                                </div>
                            }
                            words={
