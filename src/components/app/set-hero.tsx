 ...
 * full at the top and gone at the bottom, so the band is the page's top edge lit up rather than a
 * box on the page. The page header lays it edge to edge (`hero`). A logo the colour cannot be read
 * from (grey, or a catalogue not answering) gets the page's quiet grey the same way; a set
...
/** Bright at the top, nothing at the bottom: the same wash on the band and on a shelf tile. */
export const colorWash = (color: string | null) => `linear-gradient(to bottom, ${color ?? "var(--color-bg-secondary)"}, transparent)`;

export function SetHero({ name, logoUrl, color }: { name: string; logoUrl: string | null; color: string | null }) {
    return (
        <div aria-hidden="true" className="flex h-52 w-full items-center justify-center sm:h-64" style={{ background: colorWash(color) }}>
            {logoUrl ? (
