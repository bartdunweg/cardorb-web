...
export function GotItButton({
    ...
    card: Card;
    /** The mark alone, where a tile is too narrow for the words. */
    compact?: boolean;
    className?: string;
}) {
    ...
    return (
        <MarkOwnedDialog card={card} folders={folders ?? []} languages={facts?.facts?.languages} facts={facts?.facts}>
            <Button size="sm" color="secondary" iconLeading={Check} aria-label={`Got it: ${card.name}`} onClick={load} className={className}>
                {compact ? null : "Got it"}
            </Button>
        </MarkOwnedDialog>
    );
