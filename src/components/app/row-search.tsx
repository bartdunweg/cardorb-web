...
// replaces the round button of #519). 44 px high on a phone, as the buttons under it are; 36 from sm. A long list name ends in an
// ellipsis in the 208 px field rather than being cut mid-letter.
...
export function RowSearch({ children }: { children: ReactNode }) {
    return (
        <div className="flex min-w-0 basis-full items-center sm:max-w-52 sm:flex-1 sm:basis-auto [&_input]:text-ellipsis max-sm:[&_input]:h-11">{children}</div>
    );
}
