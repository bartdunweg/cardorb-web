...
interface TagGroupProps extends AriaTagGroupProps, RefAttributes<HTMLDivElement> {
    label: string;
    size?: "sm" | "md" | "lg";
    /** A box in each tag while tags can be selected. Default: whenever they can. */
    checkboxes?: boolean;
}

export const TagGroup = ({ label, selectionMode = "none", size = "sm", checkboxes = selectionMode !== "none", children, ...otherProps }: TagGroupProps) => {
    return (
        <TagGroupContext.Provider value={{ selectionMode, size, checkboxes }}>
        ...
        <AriaTag
            ...
            className={(state) =>
                cx(
                    ...
                    // With checkbox
                    context.checkboxes && styles[context.size].root.withCheckbox,
                    // Disabled
            ...
            {({ isSelected, isDisabled, allowsRemoving }) => (
                <>
                    <div className={cx("flex items-center gap-1", styles[context.size].content)}>
                        {context.checkboxes && <TagCheckbox size={context.size} isSelected={isSelected} isDisabled={isDisabled} />}
