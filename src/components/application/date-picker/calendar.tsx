...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // react-aria-components 1.20 widened the context's onChange to also accept date ranges
    // (arrays). This provider is single-date only, so the narrower setter is safe to cast.
    const onChangeValue = onChange as (value: DateValue | readonly DateValue[]) => void;

    return <AriaCalendarContext.Provider value={{ value, onChange: onChangeValue, focusedValue, onFocusChange }}>{children}</AriaCalendarContext.Provider>;
};
