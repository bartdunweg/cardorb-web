...
// Rectangular thumbnail (e.g. a trading card), keeping the source's aspect ratio.
interface CommandDropdownMenuItemImageType extends CommandMenuItemBaseType {
    type: "image";
    src: string | null;
    alt: string;
}
...
export type CommandDropdownMenuItemType =
    ...
    | CommandDropdownMenuItemImageType
                ...
                >
                    ...
                    {props.type === "image" &&
                        (props.src ? (
                            ...
                            <img src={props.src} alt={props.alt} className={cx("mr-2 w-auto shrink-0 rounded", stacked ? "h-14" : "h-10")} loading="lazy" />
                        ) : (
                            <div className={cx("mr-2 shrink-0 rounded bg-quaternary", stacked ? "h-14 w-10" : "h-10 w-7")} />
                        ))}
